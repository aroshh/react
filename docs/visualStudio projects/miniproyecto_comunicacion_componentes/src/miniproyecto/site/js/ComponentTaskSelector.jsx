...
let ComponentTaskSelector = React.createClass ({ 
    ...
    componentDidMount: function(){
        ...
        /// Eventos
        /////////////        
        $(ComponentTaskSelectorEvents).on('loadData', function(e) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// Components - seleccionados y agrupados obtenidos del JSON ya filtrado y agrupado
////////////////////////////////////////////////////////////////////////////////////////////////////
                    ...
                    {thisGroup.map(function(group,i){
                        return(
                                <div className='level2' key={i}>
                                    ...
                                    <CallSelectorTest test={group.tests}>
                                    </CallSelectorTest >
                                </div>
                            ) 
                        })
...
let CallSelectorTest = React.createClass({
    getInitialState: function(){
        console.log('[CallSelectorTest].[getInitialState] - Establecido estado inicial');
        return {tests:[]}
    },

    render(){
        console.log('[CallSelectorTest].[render] - Montamos el objeto en el DOM virtual. Al finalizar se refleja en el DOM real.');
        //console.log('[CallSelectorTest].[render]   -> Estado groups.length '+ this.props.tests.length);
        let myStyle = {listStyle:'none'};
                                        ...
                                        <a className='btn btn-danger DeleteEnsayo' data-original-title='Eliminar el Ensayo' data-toggle='tooltip' title='' data-componentid='' data-groupid='' data-testid=''><i className='icon-remove'>Eliminar</i></a>
                                            ...
                                            <div className={'containerensayo_'} className='accordion-body in collapse' style={{height:'auto'}}>
                                                <div className='accordion-inner block-content'>
                                                    ...
                                                    <CallSelectorTask task={test.tasks}></CallSelectorTask>
                                                </div>
                                            </div>
...
let CallSelectorTask = React.createClass({
    getInitialState: function(){
        console.log('[CallSelectorTask].[getInitialState] - Establecido estado inicial');
        return {tasks:[]}
    },
    render(){
        console.log('[CallSelectorTask].[render] - Montamos el objeto en el DOM virtual. Al finalizar se refleja en el DOM real.');
        //console.log('[CallSelectorTask].[render]   -> Estado groups.length '+ this.state.task.length);
        let myStyle = {listStyle:'none'};
...
let CallSelectorTaskDetail = React.createClass({
    render(){
        console.log('[CallSelectorTaskDetail].[render] - Montamos el objeto en el DOM virtual. Al finalizar se refleja en el DOM real.');
        //console.log('[CallSelectorTaskDetail].[render]   -> Estado groups.length '+ this.state.task.length);        
        return(                
...
let ComponentTaskAgrupation = React.createClass({
    ...
    handleChange: function(e) {
        ...
        console.log("[ComponentTaskAgrupation].[onHandleChange] - Valor de 'Description' del Select "+selectOption); 
    },
    componentDidMount: function(){
...
function filtrarPorAgrupacion(selectOption, dateBefore, dateAfter){
    // InfoComponentes; //fichero JSON en /site/js/InfoComponentes.json
